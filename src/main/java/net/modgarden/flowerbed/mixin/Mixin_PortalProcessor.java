...
public class Mixin_PortalProcessor {
	...
	private boolean flowerbed$cancelPortalTeleportation(boolean original, @Local(argsOnly = true) ServerLevel serverLevel) {
		if (!serverLevel.getServer().getGameRules().getBoolean(FlowerbedGamerules.ALLOW_ENTERING_PORTALS) && (portal instanceof NetherPortalBlock || portal instanceof EndPortalBlock)) {
			return false;
		}
		return original;
	}
